...
class OrderController extends ParentController
{
    public function index()
    {
       ...
       return view('pages.user.order.index')->with($response);
    }
    public function new()
    {
       ...
       return view('pages.user.order.new')->with($response);
    }
    public function view($id)
    {
       $response['order']=OrderFacade::get($id);
       return view('pages.user.order.view')->with($response);
    }
       /**
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('user.order')->with('alert-success', 'Order Added Successfully');
    }
    ...
    public function addStore($id)
    {
        ...
        foreach ($order->orderItems as  $item) {
            ...
            if ( $item->store_status==0) {
                ...
                return redirect()->route('user.product-store')->with('alert-warning', 'Already Added');
            }

        }
        OrderFacade::update($id,2);
        return redirect()->route('user.product-store')->with('alert-success', 'Order Added to Store Successfully');
    }
    ...
    public function approveOrder($id)
    {
        OrderFacade::approve($id);
         return redirect()->route('user.order')->with('alert-success', 'Order Approved Successfully');;
    }

    public function completeOrder($id)
    {
        OrderFacade::update($id,3);
         return redirect()->route('user.order')->with('alert-success', 'Order Completed Successfully');;
    }
}
